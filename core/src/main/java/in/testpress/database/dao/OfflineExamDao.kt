...
interface OfflineExamDao: BaseDao<OfflineExam> {
    ...
    @Query("UPDATE OfflineExam SET offlinePausedAttemptsCount = :pausedAttemptsCount WHERE id = :examId")
    suspend fun updateOfflinePausedAttemptCount(examId: Long, pausedAttemptsCount: Long)
    ...
    @Query("UPDATE OfflineExam SET attemptsCount = attemptsCount - :attemptsCount WHERE id = :examId")
    suspend fun reduceAttemptCount(examId: Long, attemptsCount: Long)
}