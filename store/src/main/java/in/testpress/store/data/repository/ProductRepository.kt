...
import `in`.testpress.store.data.database.TestpressStoreDatabase
...
class ProductRepository(val context: Context) {
    ...
    val storeDatabase = TestpressStoreDatabase.invoke(context)
    var page = 1
    val category = -1
    ...
    private fun deleteExistingProducts() {
        CoroutineScope(Dispatchers.IO).launch {
            storeDatabase.productLiteEntityDao().deleteAll()
        }
    }

    private fun updateProductsResource() {
        CoroutineScope(Dispatchers.IO).launch {
            val products = storeDatabase.productLiteEntityDao().getAll()
    ...
    private fun storeContent(response: List<NetworkProductLite>) {
        CoroutineScope(Dispatchers.IO).launch {
            storeDatabase.productLiteEntityDao().insertAll(response.asDomain())
        }
    }
