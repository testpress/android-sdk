...
class OfflineAttachmentsRepository(private val dao: OfflineAttachmentsDao) {
    ...
    suspend fun updateStatus(id: String, status: OfflineAttachmentDownloadStatus) = dao.updateStatus(id, status)
    suspend fun updateProgress(id: String, progress: Int) = dao.updateProgress(id, progress)
}